...
export default function Services() {
  return (
    <section id="servizi" className="py-20 bg-gradient-to-b from-beige-50 to-beige-100">
          ...
          {services.map((service, index) => {
            const CardContent = (
              ...
              >
                ...
                {/* Overlay */}
                <div className="absolute inset-0 bg-gradient-to-t from-beige-800/95 via-beige-700/70 to-transparent"></div>

                {/* Content */}
                <div className="relative p-8 min-h-[400px] flex flex-col justify-end">
                  <div className="text-5xl mb-4">{service.icon}</div>
                  <h3 className="text-2xl md:text-3xl font-bold text-beige-50 mb-3">
                    {service.title}
                  </h3>
                  <p className="text-beige-100 leading-relaxed mb-4">
                    {service.description}
                  </p>
                  {service.link && (
                    <span className="text-bordeaux-200 font-medium inline-flex items-center group-hover:text-beige-50 transition-colors">
                      Scopri di più →
                    </span>
                  )}
                </div>
